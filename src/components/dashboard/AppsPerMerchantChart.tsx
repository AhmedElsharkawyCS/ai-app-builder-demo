...
import { useAppsPerMerchant } from '../../queries/dashboard.queries';
import { WaveOverlay } from './WaveOverlay';
import { APPS_PALETTE } from '../../utils/chartColors';
import type { TimePeriod } from '../../types/dashboard.types';

interface AppsPerMerchantChartProps {
  timePeriod: TimePeriod;
}

export function AppsPerMerchantChart({ timePeriod }: AppsPerMerchantChartProps) {
  const { data, isLoading, isError, refetch } = useAppsPerMerchant(timePeriod);
  ...
  return (
    ...
    >
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 2.5 }}>
        ...
        >
          <AppsIcon sx={{ color: '#fff', fontSize: 18 }} />
        </Box>
        <Typography variant="subtitle1" sx={{ fontWeight: 700 }}>
          Apps Used per Merchant
        </Typography>
      </Box>
      ...
      {isLoading ? (
        ...
        <Alert severity="error" action={<Button size="small" onClick={() => refetch()}>Retry</Button>}>
          Failed to load apps per merchant
        </Alert>
      ) : chartData.length === 0 ? (
        ...
        <Box sx={{ width: '100%', height: Math.max(280, chartData.length * 36), position: 'relative', zIndex: 1 }}>
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={chartData} layout="vertical" margin={{ top: 4, right: 24, left: 8, bottom: 4 }}>
              <defs>
                {chartData.map((_, i) => (
                  <linearGradient key={i} id={`appsPerMerchant-${i}`} x1="0" y1="0" x2="1" y2="0">
                    <stop offset="0%" stopColor={APPS_PALETTE[i % APPS_PALETTE.length]} stopOpacity={0.85} />
                    <stop offset="100%" stopColor={APPS_PALETTE[i % APPS_PALETTE.length]} stopOpacity={1} />
                  </linearGradient>
                ))}
              </defs>
              ...
              <Tooltip
                ...
                cursor={{ fill: isLight ? 'rgba(20,125,197,0.06)' : 'rgba(61,166,225,0.08)' }}
              />
              <Bar
                dataKey="apps"
                radius={[0, 6, 6, 0]}
                ...
                shape={(props: any) => {
                  ...
                  return (
                    <rect
                      ...
                      height={height}
                      rx={6}
                      fill={`url(#appsPerMerchant-${index})`}
                    />
                  );
                }}
              />
      ...
      <WaveOverlay color={isLight ? '#147dc5' : '#3da6e1'} opacity={isLight ? 0.04 : 0.06} height={45} />
    </Paper>
  );
}